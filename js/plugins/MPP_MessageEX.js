...
(() => {
    ...
    Window_Base.prototype.convertEscapeCharacters = function(text) {
		...
		.replace(/\x1b([SIWATE])(I?)(D?)N\[(\d+)\]/gi, (_, p1, p2, p4, p3) => {
                return this.mppConvertItemName(p1, p2 !== '', p4, Number(p3));
            //.replace(/\x1b([SIWA])(I?)N\[(\d+)\]/gi, (_, p1, p2, p3) => {
            //    return this.mppConvertItemName(p1, p2 !== '', Number(p3));
            });
    };

    Window_Base.prototype.mppConvertItemName = function(type, icon, desc, itemId) {
        let text = '';
        let item;
        switch (type.toUpperCase()) {
            ...
            case 'A':
                ...
                break;
            case 'T':
                item = $dataStates[itemId];
                break;
            case 'E':
                item = $dataEnemies[itemId];
                break;
        }
        if (item) {
            if (icon) text += `\x1bI[${item.iconIndex}]`;
		if (desc) text += item.description;
		else text += item.name;
