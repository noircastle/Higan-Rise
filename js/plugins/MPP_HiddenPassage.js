 ...
 *   - Of the region IDs set in the plugin parameters,
 *     regions below this number are displayed below the player.
 ...
 *   - Tiles with the set region ID and higher than the player's Z coordinate
 *     are displayed above the character.
 ...
 * 
 * ▼ Important point
 *  - According to the specifications on the Maker side, the upper part
 *    of the A4 (wall) of the tile set is passable.
 *  - When creating secret passages, it is recommended to make both sides
 *    accessible and impassable.
 * 
 * ================================
 ...
 *      @desc An array of region IDs displayed above and passable
 * (Range can be specified)
 *      @default 32
 *
 *  @param Impassable Region Ids
 *      @desc An array of region IDs that are displayed above and cannot be passed
 * (Range can be specified)
 *      @default 33
 *
 */
 ...
 * 
 * ▼ 注意点
 *  - ツクール側の仕様で、タイルセットのA4(壁)は上部が通行可能となっています。
...
(() => {
    ...
    Object.assign(_mzCommands, {
        'プレイヤーZ座標設定': _mzCommands.SetPlayerZ
    });
    ...
    PluginManager.registerCommand(pluginName, 'setPlayerZ', args => {
